...
// Intersect x slabs
'float invRayDir = 1.0 / ray.direction.x;', 'float tNear = (p0.x - ray.origin.x) * invRayDir;', 'float tFar  = (p1.x - ray.origin.x) * invRayDir;', 'float temp = min(tNear, tFar);', 'tFar = max(tNear, tFar);', 'tNear = temp;', 't0 = mix(tNear, t0, step(0.5, float(tNear > t0)));', 't1 = mix(tFar, t1, step(0.5, float(tFar < t1)));', 'if (t0 > t1) { return -1.0; }',

// Intersect y slabs
'invRayDir = 1.0 / ray.direction.y;', 'tNear = (p0.y - ray.origin.y) * invRayDir;', 'tFar  = (p1.y - ray.origin.y) * invRayDir;', 'temp = min(tNear, tFar);', 'tFar = max(tNear, tFar);', 'tNear = temp;', 't0 = mix(tNear, t0, step(0.5, float(tNear > t0)));', 't1 = mix(tFar, t1, step(0.5, float(tFar < t1)));', 'if (t0 > t1) { return -1.0; }',

// Intersect z slabs
'invRayDir = 1.0 / ray.direction.z;', 'tNear = (p0.z - ray.origin.z) * invRayDir;', 'tFar  = (p1.z - ray.origin.z) * invRayDir;', 'temp = min(tNear, tFar);', 'tFar = max(tNear, tFar);', 'tNear = temp;', 't0 = mix(tNear, t0, step(0.5, float(tNear > t0)));', 't1 = mix(tFar, t1, step(0.5, float(tFar < t1)));', 'if (t0 > t1) { return -1.0; }',
...
'return mix(t0, t1, step(0.5, float(t0 > 0.0)));', '}'].join('\n');
...
var Raytracer = function () {
    ...
    createClass(Raytracer, [{
        ...
        value: function makeFragmentShader(ptrsSize, piSize, ptrsWidth, piWidth) {
            ...
            var fShader = ['varying vec4 vPosition;', 'uniform vec3 lightsource;', 'uniform float lightIntensity;', 'uniform float ambientIntensity;', 'uniform vec2 xBounds;', 'uniform vec2 yBounds;', 'uniform vec2 zBounds;', 'uniform sampler2D primitive_ptrs;', 'uniform sampler2D primitive_info;', 'const int MAX_BOUNCES = 4;', 'const int POINTERS_SIZE = ' + ptrsSize + ';', 'const int PI_SIZE = ' + piSize + ';', 'const int POINTERS_WIDTH = ' + ptrsWidth + ';', 'const int PI_WIDTH = ' + piWidth + ';', 'const float PIXEL_WIDTH_PTRS = 1./float(POINTERS_WIDTH);', 'const float PIXEL_WIDTH_INFO = 1./float(PI_WIDTH);', 'float refractionIndex = 1.;', glsl.structs.ray, glsl.structs.sphere, glsl.structs.triangle, glsl.structs.box, glsl.intersectFunctions.intersectSphere, glsl.intersectFunctions.intersectTriangle, glsl.intersectFunctions.intersectBox,
            ...
            'if (pInfo_min.g == 1.) {', 'currentRay.origin += min_t * currentRay.direction;', 'except = pid;', 'pid = -1;', 'min_t = 100000.;',
            // sphere case
            'if (pInfo_min.r > 0.5) {', 'vec3 c = texture2D(primitive_info, vec2((pIdx_min+3.+0.5)*PIXEL_WIDTH_INFO, 0.5)).xyz;', 'normal = normalize(currentRay.origin-c);', '} else if (pInfo_min.r < 0.5) {', 'vec3 a = texture2D(primitive_info, vec2((pIdx_min+3.+0.5)*PIXEL_WIDTH_INFO, 0.5)).xyz;', 'vec3 b = texture2D(primitive_info, vec2((pIdx_min+4.+0.5)*PIXEL_WIDTH_INFO, 0.5)).xyz;', 'vec3 c = texture2D(primitive_info, vec2((pIdx_min+5.+0.5)*PIXEL_WIDTH_INFO, 0.5)).xyz;', 'normal = normalize(cross(b - a, c - a));', '}', 'currentRay.direction -= 2.*normal * dot(currentRay.direction, normal);', '} else if (pInfo_min.g == 2.) {',
            ...
            '} else {', 'ri_new = pInfo_min.b;', '}', 'currentRay.origin += min_t*currentRay.direction;', 'refracted_pid = pid;', 'pid = -1;', 'except = -1;', 'min_t = 100000.;', 'if (pInfo_min.r > 0.5) {', 'vec3 c = texture2D(primitive_info, vec2((pIdx_min+3.+0.5)*PIXEL_WIDTH_INFO, 0.5)).xyz;', 'normal = normalize(currentRay.origin -c);', '} else if (pInfo_min.r < 0.5) {', 'vec3 a = texture2D(primitive_info, vec2((pIdx_min+3.+0.5)*PIXEL_WIDTH_INFO, 0.5)).xyz;', 'vec3 b = texture2D(primitive_info, vec2((pIdx_min+4.+0.5)*PIXEL_WIDTH_INFO, 0.5)).xyz;', 'vec3 c = texture2D(primitive_info, vec2((pIdx_min+5.+0.5)*PIXEL_WIDTH_INFO, 0.5)).xyz;', 'normal = normalize(cross(b - a, c - a));', '}', 'if (dot(normal, currentRay.direction) >= 0.) { normal = -normal; }', 'currentRay.direction = refract(currentRay.direction, normal, refractionIndex/ri_new);', 'currentRay.origin += currentRay.direction * 0.001;', 'refractionIndex = ri_new;', '} else {', 'return;', '}', '}', '}', 'void main() {', 'vec3 ro = cameraPosition;', 'vec3 dir = vPosition.xyz - ro;', 'Ray ray = Ray(cameraPosition, normalize(dir));', 'float t_entry = length(dir);', 'if (t_entry < 0.) { gl_FragColor = vec4(0.,0.,0.,1.); return; }', 'vec3 color = vec3(0.,0.,0.);', 'intersect(ray, color);', 'gl_FragColor = vec4(color, 1.0);', '}'].join('\n');
